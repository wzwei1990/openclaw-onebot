...
# 方式二：手动安装
cp -r src scripts index.ts package.json tsconfig.json ~/.openclaw/plugins/onebot/
...
默认共享目录：
- `sharedDir`: `~/napcat/shared`
- `containerSharedDir`: `/shared`
- CLI / setup 支持 `--token wsUrl,httpUrl[,accessToken[,sharedDir[,containerSharedDir]]]`
- OpenClaw 升级后如需重新接线，可执行 `npm run sync:openclaw-cli`
...
| 参数 | 说明 |
|------|------|
| `allowFrom` | 消息来源过滤 — `private:<QQ号>`、`group:<群号>`、或 `*` |
| `accessToken` | HTTP API 使用 `Authorization: Bearer <token>`，WebSocket 使用 query 参数 |
| `sharedDir` | 宿主机共享目录；默认 `~/napcat/shared` |
| `containerSharedDir` | 容器内共享目录；默认 `/shared` |
| `groupAutoReact` | 是否对入站群消息自动 reaction，默认 `true` |
| `groupAutoReactEmojiId` | 自动 reaction 使用的 QQ emoji id，默认 `1` |
...
Default shared-dir settings:
- `sharedDir`: `~/napcat/shared`
- `containerSharedDir`: `/shared`
- CLI / setup also support `--token wsUrl,httpUrl[,accessToken[,sharedDir[,containerSharedDir]]]`
- Re-run `npm run sync:openclaw-cli` after an OpenClaw upgrade if the installed CLI dist gets replaced

| Option | Description |
|--------|-------------|
| `allowFrom` | Filter inbound — `private:<qq>`, `group:<id>`, or `*` |
| `accessToken` | Bearer token for HTTP API, query param for WebSocket |
| `sharedDir` | Host-side shared directory; defaults to `~/napcat/shared` |
| `containerSharedDir` | Container-side mount path; defaults to `/shared` |
